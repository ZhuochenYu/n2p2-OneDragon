        ...
        with open('input.nn', 'r') as nn_f:
            for line in nn_f:
                if line.startswith('elements'):
                    elements = line.split('#')[0].replace('elements','')
                    self.elements = elements.split()
                    print(self.elements)
        
    def generate_symfunc(self,symfunc_type):
        if os.path.exists('symfunc.out'):
            os.remove('symfunc.out')
        myGenerator = SymFuncParamGenerator(elements=self.elements, r_cutoff = 6.)
        self.symfunc_type = symfunc_type.split()
        print('Gnerating symfunc.out')
        for type in self.symfunc_type:
            if type == 'radial':
                myGenerator.symfunc_type = 'radial'
                myGenerator.generate_radial_params(rule='imbalzano2018', mode='shift', nb_param_pairs=5)
                with open('symfunc.out', 'a') as f:
                    myGenerator.write_settings_overview(fileobj=f)
                    myGenerator.write_parameter_strings(fileobj=f)
            elif type == 'angular_wide':
                ...
                with open('symfunc.out', 'a') as f:
                    myGenerator.write_settings_overview(fileobj=f)
                    myGenerator.write_parameter_strings(fileobj=f)
        print("Finish generating symfunc.out")
        print("Modifying input.nn, and symfunc.out will be removed.")
        self.symfunc_append()
        os.remove('symfunc.out')
        print("All preparations have been completed.")
    def symfunc_append(self):
        with open('symfunc.out', 'r') as symfunc_f:
            symfunc_content = symfunc_f.readlines()
        with open('input.nn', 'r') as nn_f:
            nn_content = nn_f.readlines()
        index = next((i for i, line in enumerate(nn_content) if "# EXAMPLE SYMMETRY FUNCTIONS #" in line), None)
        if index is not None:
        # delete 
            nn_content = nn_content[:index]
        nn_content.extend(symfunc_content)
        with open('input.nn', 'w') as file:
            file.writelines(line for line in nn_content)
